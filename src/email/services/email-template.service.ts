...
export class EmailTemplateService {
  private getBaseTemplate({ title, headerText, content }: BaseEmailTemplate): string {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  createWelcomeEmailTemplate({ name, profileUrl }: WelcomeEmailProps): string {
    const content = `
  ...
  createConnectionAcceptedTemplate({ senderName, recipientName, profileUrl }: ConnectionAcceptedEmailProps): string {
    const content = `
